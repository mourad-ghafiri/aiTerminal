...
pub(crate) const BUILTINS: &[SlashCommand] = &[
    ...
    SlashCommand { name: "/compact", about: "fold the history down now; /compact <what to keep> steers the summary" },
    SlashCommand { name: "/mode", about: "cycle plan \u{2192} build \u{2192} auto (shift+tab does the same)" },
    SlashCommand { name: "/plan", about: "plan mode: read-only exploring, then a plan you approve" },
    SlashCommand { name: "/build", about: "build mode: the full toolset, confirms ask you" },
    SlashCommand { name: "/auto", about: "auto mode: the AI judge answers confirms; you, when it declines" },
    ...
    SlashCommand { name: "/guard", about: "the active protections; /guard <cmd> asks for a verdict without running it" },
...
pub(crate) enum Route {
    ...
    Init,
    Clear,
    /// Fold history now; the argument names what the summary must keep.
    Compact(Option<String>),
    /// Switch the working mode: cycle (`None`) or jump to a named one.
    Mode(Option<super::screen::Mode>),
    Model(Option<String>),
    ...
    Learn,
    Changes,
    /// The guard's briefing (`None`) or a dry-run verdict on an act.
    Guard(Option<String>),
    Exit,
...
pub(crate) fn route(line: &str, prompts: &[crate::ai::defs::Prompt], agents: &[String]) -> Route {
    ...
    if trimmed.starts_with('/') {
        ...
        return match head {
            ...
            "/compact" => Route::Compact((!rest.is_empty()).then(|| rest.to_string())),
            "/mode" => Route::Mode(None),
            "/plan" => Route::Mode(Some(super::screen::Mode::Plan)),
            "/build" => Route::Mode(Some(super::screen::Mode::Build)),
            "/auto" => Route::Mode(Some(super::screen::Mode::Auto)),
            ...
            "/guard" => Route::Guard((!rest.is_empty()).then(|| rest.to_string())),
