...
impl Regex {
    ...
    pub fn replace_all_opt(&self, text: &str, replacement: &str) -> Option<String> {
        self.replace_all_with(text, &mut |_| replacement.to_string())
    }

    /// Like [`replace_all_opt`](Self::replace_all_opt), but the replacement is chosen per
    /// match from the text that matched.
    ///
    /// This is what a **reversible** redaction needs: the same secret has to become the
    /// same placeholder everywhere, and a different secret a different one, which a single
    /// fixed replacement string cannot express. The scan, the step budget and the
    /// empty-match handling are shared with the fixed form — one loop, so a rule can never
    /// behave differently depending on which caller asked.
    pub fn replace_all_with(&self, text: &str, with: &mut dyn FnMut(&str) -> String) -> Option<String> {
        if self.prefiltered_out(text) {
            return None;
        }
        let chars: Vec<char> = text.chars().collect();
        ...
        while i <= chars.len() {
            match self.find_in(&chars, i, &steps, budget) {
                FindResult::Match(s, e) => {
                    ...
                    out.push_str(&with(&chars[s..e].iter().collect::<String>()));
                    if e > s {
                        i = e;
                    } else {
