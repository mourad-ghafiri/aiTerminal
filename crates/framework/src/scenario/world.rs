            ...
            Step::ExpectSays(want) => {
                ...
                for w in want {
                    if !all.contains(w.as_str()) {
                        return Err(format!("expected {w:?} in what was said; got {}", show(&all)));
                    }
                }
                self.said.clear();
            }
            Step::ExpectNotSays(bad) => {
                ...
                for b in bad {
                    if all.contains(b.as_str()) {
                        return Err(format!("{b:?} must NOT have been said; got {}", show(&all)));
                    }
                }
            }
            Step::ExpectPty(want) => {
                ...
                if got != *want {
                    return Err(format!("terminal received {} — expected {}", show(&got), show(want)));
                }
                self.pty.clear();
            }
            Step::ExpectNoPty => {
                if !self.pty.is_empty() {
                    let got = String::from_utf8_lossy(&self.pty).into_owned();
                    return Err(format!("nothing should have reached the terminal, but {} did", show(&got)));
                }
            }
            ...
            Step::ExpectLocal(want) => {
                for w in want {
                    if !self.local.contains(w.as_str()) {
                        return Err(format!("expected {w:?} in the pane; got {}", show(&self.local)));
                    }
                }
                self.local.clear();
            }
...
/// Make control bytes visible in a failure message.
