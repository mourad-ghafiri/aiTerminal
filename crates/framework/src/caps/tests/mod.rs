...
fn ctx() -> CapCtx {
    CapCtx { guard: plain_guard(), app_data: None, remote_enabled: true, origin: String::new(), sandbox: None, memory_dir: None, approver: std::sync::Arc::new(crate::guard::NobodyToAsk), asker: std::sync::Arc::new(crate::caps::ask::NobodyToAnswer) }
}
...
fn ctx_ws(root: &std::path::Path) -> CapCtx {
    CapCtx { guard: plain_guard(), app_data: None, remote_enabled: true, origin: String::new(), sandbox: Some(root.to_path_buf()), memory_dir: None, approver: std::sync::Arc::new(crate::guard::NobodyToAsk), asker: std::sync::Arc::new(crate::caps::ask::NobodyToAnswer) }
}
...
fn web_read_md_fixture_and_remote_guards() {
    ...
    let no_remote = CapCtx { guard: plain_guard(), app_data: None, remote_enabled: false, origin: String::new(), sandbox: None, memory_dir: None, approver: std::sync::Arc::new(crate::guard::NobodyToAsk), asker: std::sync::Arc::new(crate::caps::ask::NobodyToAnswer) };
...
fn sys_run_guard_sees_canonical_argv_not_quoted_bypass() {
    ...
    let ctx = CapCtx { guard: Arc::new(guard), app_data: None, remote_enabled: true, origin: String::new(), sandbox: None, memory_dir: None, approver: std::sync::Arc::new(crate::guard::NobodyToAsk), asker: std::sync::Arc::new(crate::caps::ask::NobodyToAnswer) };
...
fn store_requires_app_sandbox() {
    ...
    let _ = std::fs::remove_dir_all(&dir);
    let c = CapCtx { guard: plain_guard(), app_data: Some(dir.clone()), remote_enabled: true, origin: String::new(), sandbox: None, memory_dir: None, approver: std::sync::Arc::new(crate::guard::NobodyToAsk), asker: std::sync::Arc::new(crate::caps::ask::NobodyToAnswer) };
...
fn ctx_offline() -> CapCtx {
    CapCtx { guard: plain_guard(), app_data: None, remote_enabled: false, origin: String::new(), sandbox: None, memory_dir: None, approver: std::sync::Arc::new(crate::guard::NobodyToAsk), asker: std::sync::Arc::new(crate::caps::ask::NobodyToAnswer) }
}
...
fn asking_the_guard_about_a_path_gets_the_same_answer_the_tool_would_give() {
    ...
    let c = CapCtx { guard: plain_guard(), app_data: None, remote_enabled: false, origin: String::new(), sandbox: Some(dir.clone()), memory_dir: None, approver: std::sync::Arc::new(crate::guard::NobodyToAsk), asker: std::sync::Arc::new(crate::caps::ask::NobodyToAnswer) };
    let pem = dir.join("server.pem");
...
fn a_command_that_only_becomes_dangerous_once_a_secret_is_put_back_never_runs() {
    ...
    let c = CapCtx { guard: Arc::new(guard), app_data: None, remote_enabled: false, origin: String::new(), sandbox: Some(ws.clone()), memory_dir: None, approver: std::sync::Arc::new(crate::guard::NobodyToAsk), asker: std::sync::Arc::new(crate::caps::ask::NobodyToAnswer) };
...
fn a_memory_keeps_neither_the_secret_nor_a_placeholder() {
    ...
    let guard = crate::guard::Guard::from_toml("[[guard.secret]]\npattern = \"pw-[a-z0-9]+\"\nname = \"db-password\"\n");
    let token = guard.hide("pw-example0");
    let c = CapCtx { guard: Arc::new(guard), app_data: None, remote_enabled: false, origin: String::new(), sandbox: None, memory_dir: Some(store.clone()), approver: std::sync::Arc::new(crate::guard::NobodyToAsk), asker: std::sync::Arc::new(crate::caps::ask::NobodyToAnswer) };
...
fn a_confirm_rule_asks_the_approver_and_a_deny_never_does() {
    ...
    let make = |answer| {
        ...
        let ctx = CapCtx {
            ...
            asker: std::sync::Arc::new(crate::caps::ask::NobodyToAnswer),
        };
        (ctx, approver)
    };
...
fn headless_contexts_still_refuse_a_confirm() {
    ...
    let ctx = CapCtx {
        ...
        sandbox: None,
        memory_dir: None,
        approver: std::sync::Arc::new(crate::guard::NobodyToAsk), asker: std::sync::Arc::new(crate::caps::ask::NobodyToAnswer),
    };
    let err = ctx.allow(crate::guard::Act::Run("touch-the-config now")).unwrap_err();
    assert!(crate::guard::is_refusal(&err), "{err}");
}
