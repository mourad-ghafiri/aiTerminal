...
| 📄 `@md <file>` | `@md render` pretty-prints a Markdown file (**every mermaid diagram type** drawn natively — flowchart, sequence, class, state, ER, gantt, pie, journey, mindmap, git graph, …; box art in other terminals); `@md edit` is a live split editor — Markdown left, rendered preview right, scroll by keyboard + mouse. |
